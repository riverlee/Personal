...
my $strand=0;     # 0 for no strand,1 for forward, -1 for reverse 
...
if($isalnbam){
    ...
    while(<IN>){
        ...
        # only forward strand
        if($strand == 1){
            next if ($flag & $flag_reverse_strand);
        }

        if($strand ==-1){
            next unless ($flag & $flag_reverse_strand);
        }
...
#        ($start,$end)=sort {$a<=>$b} ($start,$end); #in case $end < $start
    ...
    while(<IN>){
        ...
        my($chr,$start,$end,@a) = split "\t";
        if($strand ==1 && scalar(@a)>=3){
            next if ($a[2] eq "-");
        }
...
Usage: getCenteredSig.pl -a <alignfile> -c <centerfile> -o <output> -e [extend bp] -w [window size] -n/-non
                ...
                -s  strand, values are[-1,0,1],-1 for reverse strand,0 no strand specified,1 for forward strand, default is 0
